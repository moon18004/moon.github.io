...
fetch(apiWeatherURL)
    ...
    .then(
        (currentWeather) => {
            ...
            document.getElementById("wind_speed").innerHTML = windSpeed + ' MPH';


            if (windChill === 0) {
                document.getElementById("wind_chill").innerHTML = 'N/A';
            } else {
                document.getElementById("wind_chill").innerHTML = Math.round(windChill) + '&deg;';
            }
        }
    );
...
fetch(apiweatherURL)
    ...
    .then((currentWeather) => {
        ...
        document.getElementById("temp").innerHTML = Math.round(currentTemp) + '&deg;F';
        ...
        if (windChill == 0) {
            ...
            document.getElementById("wind_chill").innerHTML = Math.round(windChill) + '&deg;F';
        }
    });